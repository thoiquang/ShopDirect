...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    const card = isCard ? await DataStore.getVirtualCard() : null;
    if (isCard && !card) {
    ...
    if (isCard) {
        content.innerHTML = `<div class="shop-virtual-card mb-5"><p class="text-xs uppercase tracking-widest text-blue-100">ShopDirect Card</p><p class="text-xl tracking-widest mt-6">**** **** **** ${card.cardNumber.slice(-4)}</p><div class="flex justify-between text-xs mt-5"><span>${card.cardholderName}</span><span>${card.expiry}</span></div></div><div class="flex justify-between border-t pt-4 font-bold"><span>Số tiền thanh toán</span><span class="text-blue-700">${formatPaymentAmount(pending.totalAmount)}</span></div>`;
    } else {
